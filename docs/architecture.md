...
Some equivalent events will still have distinct identifiers. A persistent administrator-maintained equivalence table stores the internal pairs that define an equivalence graph; it does not introduce a collection ID. The administration interface presents and edits complete groups, each with an automatically suggested but manually editable title. The portable representation is the documented [event-equivalence JSON format](event-equivalence-json.md). The generated identity index is deliberately excluded from that format and is rebuilt automatically after import.
